...
//
// Description: 
//  Illustrates use of PS3GetLightWeightMutexInfo and PS3GetLightWeightMutexList
// 
//
//****************************************************************************
sn_uint32 GetLWMutexList(sn_uint32 uProcessID)
{
	...
	for (i = 0; i < uCount ; i++)
	{
		...
		printf("\nID 0x%x, AttrProtocol 0x%x AttrRecursive 0x%x name %s OwnerThread ID 0x%x, LockCounter 0x%x", pBuffer[i], pInfo->Attribute.uAttrProtocol, pInfo->Attribute.uAttrRecursive, pInfo->Attribute.aName, pInfo->uOwnerThreadID.word[0], pInfo->uLockCounter );
...
int main(int argc, char ** argv)
{
	...
	printf("This example script adds a custom menu item into a source view context menu which will print current light weight mutex information into this view\n");

	AddMenuItem("Get Light Weight Mutex List and Info", &uSrcItemId[0], "DBGSourceView");
